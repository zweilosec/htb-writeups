...
Disallow: /admin-dir
```
potential username `waldo`. Using Dirbuster on this directory led me to a few files. http://10.10.10.187/admin-dir/contacts.txt
...
have to set up a local mysql database https://www.microfocus.com/documentation/idol/IDOL_12_0/MediaServer/Guides/html/English/Content/Getting_Started/Configure/_TRN_Set_up_MySQL_Linux.htm
...
```
After trial and error, I was able to get my library to be loaded, though I did not get a shell like I expected.  However, I could see that the output of the `whoami` command did appear, so I had proof that I could run commands as root.
I got an error when trying to exploit this with version one of my evil python library, but that error also confirmed that I was making progress.  It told me that my "make_archive()" function takes in 0 positional arguments but the script that was calling it was feeding it three.  
```python
import os
